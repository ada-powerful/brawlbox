...
import { KFM2LITE_CHARACTER } from '../src/sandbox/kfm2lite.ts';

const characters = { kfm2lite: KFM2LITE_CHARACTER };

function world(): World {
  return createWorld('kfm2lite', 'kfm2lite');
}
...
describe('kfm2lite builds and validates', () => {
  ...
  test('cut moves are gone (states + animations)', () => {
    ...
    for (const a of [
      ...
      expect(KFM2LITE_CHARACTER.animations?.[a]).toBeUndefined();
    }
  });

  test('merged moves borrow another row’s animation', () => {
    const st = KFM2LITE_CHARACTER.states;
