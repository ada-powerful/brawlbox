...
// keys that DEFAULT_TEMPLATE_MANIFEST maps onto the Kyo reference sheet's rows.
...
const KYO_ANIMATIONS: Record<string, Animation> = {
  // Movement
...
// parseCharacter). All reference anim ids present in KYO_ANIMATIONS.
...
function buildKyoTemplate(): Character {
  const c = structuredClone(baseChar) as unknown as RawChar;
  c.meta = { id: 'kyo-template', name: 'Kyo (action template)', author: 'BrawlBox', version: '0.1.0' };
  c.animations = KYO_ANIMATIONS as unknown as Record<string, unknown>;
...
export const KYO_TEMPLATE: Character = buildKyoTemplate();
