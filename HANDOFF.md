...
> Written 2026-05-31, engine status refreshed 2026-06-02. Living doc for the frontend/backend split, cloud infra, and the brawlbox.gg domain. Read this first in any new session that touches infra, repos, or deployment. For **engine internals**, read `CLAUDE.md` + `MOVESET_GAP_DESIGN.md`.

## 0. TL;DR — where we are

- **Product**: `ftg` (working name) → **BrawlBox**, a browser-native 2D fighting game + AI-assisted character creator. Engine (phase 1) is complete, rollback-ready, and has since been **extended far past its original phase-1 scope** (throws, guard, charge motions, power meter, dizzy/stun, knockdowns/OTG, launch/KO flow, a full 36-action demo character, and a CPU opponent — see "Engine progress" below and `MOVESET_GAP_DESIGN.md`). Creator (phase 2) is mid-build.
- **Engine progress (2026-06-02)**: moveset-gap rounds 1–7 shipped — throws/command grabs, guard/blocking (high/low/mid/air + overheads), charge & held-direction motions, power meter, opponent triggers, dizzy/stun, launch→knockdown→getup + OTG, distinct KO/win/lose end states, a canonical ~36-action vocabulary + procedural poses + action gallery, full **KFM** (Kung Fu Man) + E. Honda demo characters, and an input-layer **CPU P2** (5 difficulty levels). Render gained a follow-camera, parallax stages, a 30s round timer, and a showcase mode. **M8 determinism stays green; 294 tests pass.**
...
- **Domain**: `brawlbox.gg` apex is **active and serving** on Cloudflare Pages. `www.brawlbox.gg` still unresolved (dashboard action — see §9 task #3).
- **Status (2026-06-01): the plan in this doc is largely EXECUTED.** Repo split done (web/api/infra all exist); AWS resources created via Terraform; backend live (generation + Cognito auth + per-user storage + public gallery); creator auto-saves and is live on brawlbox.gg. The §9 task list (#1–#9) carries per-task status. Remaining: `www` subdomain + `api.brawlbox.gg` custom domain (both DNS/dashboard actions), OAuth apps dev→live, and per-template sprite-manifest tuning.

## 1. Current codebase state (what becomes the frontend)

Repo `github.com/ada-powerful/brawlbox-web` (renamed from `ftg`), branch `main`. Stack: TS strict, Vite 6, PixiJS 8, React 19, Tailwind v4 + shadcn, Zod, Vitest, bun. **294 tests green**; `typecheck`, `test`, `build` all pass.
...
- M2.2 sprite pipeline **shipped** (was mid-pivot): per-frame gpt-image → **template retexturing via fal nano-banana-2/edit**. Pipeline now: pick a character template → NB2 re-skins a green-screen KFM sheet → content-aware grid slicing (crop each pose to its real box) → key/despill/bake → atlas → engine. `fal.ts`, `detectFrames.ts`, `templateManifest.ts` all done. Remaining: per-template manifest tuning (some poses still mis-map). See `template-creator-pipeline` memory + `PHASE_2_PLAN.md`.
...
8. ✅ **DONE (2026-06-01)** **Storage** — per-user cloud CRUD live. DynamoDB `brawlbox-characters` (pk userId, sk characterId), atlases in S3 (`characters/<userId>/<id>.png`, reusing the artifact bucket). API `/characters` GET/POST/DELETE, **auth-required** (`requireUser`, scoped to the token `sub`); list returns presigned atlas URLs. **Verified E2E** with a real token (save→list→fetch atlas→delete all 200). Frontend `src/creator/store/cloud.ts` + App. Lambda env `CHARACTERS_TABLE`/`ASSETS_BUCKET`; DDB IAM gated via `enable_dynamodb`.
   - **Public sharing/gallery DONE** (commit `b2b30c7`): `shared` BOOL + `gallery-index` GSI (gsi1pk=`PUBLIC` on shared rows), `POST /characters/:id/share` toggle, public unauth `GET /gallery` (newest shared). Frontend: Share/Unshare per row in "My characters" + a "Gallery" card; `cloud.ts` `shareCloudCharacter`/`listGallery`.
   - **Auto-save DONE** (commit `988462f`, deployed + verified in prod): generation auto-saves to the cloud and sprite-gen updates the **same** record (upsert keyed by `meta.id`); `withUniqueId` gives each generation a random `meta.id` suffix so auto-save can't clobber a prior fighter (the LLM reuses ids). Replaced the manual "Save to my collection" button with a Saving/Saved/error status line; "My characters" always shows when signed in (empty-state hint + loaded-row highlight); `refreshSaved` now surfaces load errors instead of swallowing them. Fixes the reported bug: generated character was lost on sign-out because it was never persisted.
...
10. ✅ **DONE (2026-06-02)** **Template-based character creator** — live & verified on brawlbox.gg. Flow: describe look → pick a **template** (dropdown, friendly label only; today "Bajiquan Brawler" = the 36-action **kfm2**) → NB2 re-skins the template's green-screen sheet → slice → bake onto the template's base character → auto-save → playable. The fighter REUSES the template's full gameplay (state machine, moves, hitboxes, AI); the prompt only drives the art. "Freeform (AI-designed)" keeps the legacy LLM path. Supersedes #9's "manifest tuning".
    - **Backend** (`brawlbox-api`, live `bba6c9d`+, deployed via direct `update-function-code`): `POST /generate/sprites` takes an allowlisted `templateKey`; prompt backdrop per-template (green for kfm2, legacy `default.png` stays black). `GET /templates` returns the catalog. fal: green templates render at **4K** (`aspect_ratio:'auto'`) — 2K looked blurry for realistic characters. 4K runs longer, so the **Lambda timeout was raised 120s→600s** (`update-function-configuration`). Atlas cells bumped 160→240px (template path) so 4K detail isn't downscaled away (atlas 3840×3600, < 4096 GPU limit). Legacy default path stays 2K.
    - **Slicing was the hard part** — NB2 controls neither output size/aspect (observed 1:1 2048² AND 16:9 2752×1536) nor exact layout, and **erases** grid lines drawn on the template. So fixed fractional grids, layout-pinning prompts, and magenta cell-grids ALL failed. **Working approach** (`src/creator/image/detectSlice.ts`): detect poses by their green gaps; map to keys by grid position (rows top→bottom, cols left→right — kfm2 cols are contiguous from 0); segment each row/col with `segmentByExpected` (place the KNOWN count of boundaries evenly, snap each to the nearest green valley → no frame ever spans two poses). `packAtlas.ts` `robustScale` sizes the shared scale off the 90th percentile so one over-wide crop can't shrink every sprite; `despillChroma` kills green fringe. Template carries `bg`+grid in `src/creator/templates.ts`; `templates/kfm2.png` in S3 is plain green, no grid (`src/sandbox/export-kfm2-template.py`). 302 web tests pass.
    - **Roadmap**: replace the name dropdown with gender / body-type / martial-art params → template id; use NB2 to bake a realistic, copyright-free re-skin of kfm2 as a built-in seed.
    - ⚠️ **IaC drift**: the Lambda was changed directly — `update-function-code` (code) AND `update-function-configuration --timeout 600`. Neither is in `brawlbox-infra`. Before the next `terraform apply`: bump `lambda_artifact_key` to the current `lambda/api-<sha>.zip` AND set the `lambdas` module timeout to 600 — otherwise apply reverts the code to old AND drops the timeout to ~120s, which makes 4K sprite gen time out (blank result).
