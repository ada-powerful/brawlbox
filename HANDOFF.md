...
8. ✅ **MOSTLY DONE (2026-06-01)** **Storage** — per-user cloud CRUD live. DynamoDB `brawlbox-characters` (pk userId, sk characterId), atlases in S3 (`characters/<userId>/<id>.png`, reusing the artifact bucket). API `/characters` GET/POST/DELETE, **auth-required** (`requireUser`, scoped to the token `sub`); list returns presigned atlas URLs. **Verified E2E** with a real token (save→list→fetch atlas→delete all 200). Frontend `src/creator/store/cloud.ts` + App: "Save to my collection" after sprites, "My characters" list (load via presigned URL + delete). The old IndexedDB store was removed (no migration needed). Lambda env `CHARACTERS_TABLE`/`ASSETS_BUCKET`; DDB IAM gated via `enable_dynamodb`. **TODO**: public **sharing/gallery** (a `shared` flag + GSI + `GET /gallery` + gallery UI) — not built; the table can take a GSI without recreation when needed.
