...
7. ⏳ **IN PROGRESS (2026-06-01)** **Cognito auth**: user pool + Google + Facebook IdPs (create the OAuth apps per §5), JWT authorizer on the API, login UI in the frontend. **HARD REQUIREMENT**: same-email account linking — a Google-federated user and a native email/password user with the same email must resolve to ONE account (Pre-Signup Lambda + `adminLinkProviderForUser`, verified-email-only). See user memory `cognito-account-linking`.
